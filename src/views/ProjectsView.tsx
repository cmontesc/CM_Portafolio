...
export const ProjectsView: React.FC<ProjectsViewProps> = ({ 
  ...
  const { projects, homeProjects } = usePortfolioData();
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
        <div className="space-y-3 max-w-2xl">
          <div className="inline-flex items-center gap-2 text-[12px] uppercase tracking-wider text-[#839bc8] font-normal">
            ...
            <span>Galería de Casos UX/UI · Ordenados por Año</span>
          </div>
        ...
        <div className="shrink-0 px-3.5 py-2 bg-[#f8fafd] border border-[#e5edf5] rounded-[4px] text-[13px] sm:text-[14px] text-[#64748d] font-light tracking-[-0.14px] whitespace-nowrap">
          <span className="text-[#061b31] font-normal tabular-nums">{publishedProjects.length}</span>
          <span> proyectos totales</span>
          <span className="mx-2 text-[#b9b9f9]">·</span>
          <span className="text-[#533afd] font-normal tabular-nums">{homeProjects.length}</span>
          <span> visibles en Inicio</span>
        </div>
      </div>
